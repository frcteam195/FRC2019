...
public class TalonHelper {
	public static boolean setPIDGains(TalonSRX talon, int slotID, double kP, double kI, double kD, double kF) {
		return setPIDGains(talon, slotID, kP, kI, kD, kF, Constants.kCANTimeoutMs);
	}
	...
	public static boolean setPIDGains(TalonSRX talon, int slotID, double kP, double kI, double kD, double kF, double rampRate, int iZone) {
		return setPIDGains(talon, slotID, kP, kI, kD, kF, rampRate, iZone, Constants.kCANTimeoutMs);
	}
	...
	public static boolean setMotionMagicParams(TalonSRX talon, int cruiseVelocityRPM, int maxAccelRPM) {
		return setMotionMagicParams(talon, cruiseVelocityRPM, maxAccelRPM, Constants.kCANTimeoutMs);
	}
	public static boolean setMotionMagicParams(TalonSRX talon, int cruiseVelocityRPM, int maxAccelRPM, int timeout) {
		...
		if (timeout > 0) {
			do {
				...
				setSucceeded &= talon.configMotionCruiseVelocity(QuickMaths.convertRPMToNativeUnits(cruiseVelocityRPM), timeout) == ErrorCode.OK;
				setSucceeded &= talon.configMotionAcceleration(QuickMaths.convertRPMToNativeUnits(maxAccelRPM), timeout) == ErrorCode.OK;

			} while (!setSucceeded && retryCounter++ < Constants.kTalonRetryCount);
		} else {
			talon.configMotionCruiseVelocity(QuickMaths.convertRPMToNativeUnits(cruiseVelocityRPM), timeout);
			talon.configMotionAcceleration(QuickMaths.convertRPMToNativeUnits(maxAccelRPM), timeout);
		}
		return retryCounter < Constants.kTalonRetryCount && setSucceeded;
	}

	public static boolean setMotionMagicParams(CKTalonSRX talon, int slotIdx, int cruiseVelocityRPM, int maxAccelRPM) {
		return setMotionMagicParams(talon, slotIdx, cruiseVelocityRPM, maxAccelRPM, Constants.kCANTimeoutMs);
	}
	public static boolean setMotionMagicParams(CKTalonSRX talon, int slotIdx, int cruiseVelocityRPM, int maxAccelRPM, int timeout) {
		...
		if (timeout > 0) {
			do {
				...
				setSucceeded &= talon.configMotionCruiseVelocity(QuickMaths.convertRPMToNativeUnits(cruiseVelocityRPM), slotIdx, timeout) == ErrorCode.OK;
				setSucceeded &= talon.configMotionAcceleration(QuickMaths.convertRPMToNativeUnits(maxAccelRPM), slotIdx, timeout) == ErrorCode.OK;

			} while (!setSucceeded && retryCounter++ < Constants.kTalonRetryCount);
		} else {
			talon.configMotionCruiseVelocity(QuickMaths.convertRPMToNativeUnits(cruiseVelocityRPM), slotIdx, timeout);
			talon.configMotionAcceleration(QuickMaths.convertRPMToNativeUnits(maxAccelRPM), slotIdx, timeout);
		}
		return retryCounter < Constants.kTalonRetryCount && setSucceeded;
	}
	...
	public static DiagnosticMessage checkMotorReset(BaseMotorController motorController, String name) {
		if (motorController.hasResetOccurred()) {
			...
			do {
				setSucceeded = true;
				setSucceeded &= motorController.clearStickyFaults(Constants.kCANTimeoutMs) == ErrorCode.OK;
			} while(!setSucceeded && retryCounter++ < Constants.kTalonRetryCount);
